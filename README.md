...
- Tap Secret on the start screen and enter `ilove67` to unlock the hidden Six Seven bird, enter `Logan` to unlock the flying clock with wings, or enter `Queen Lia`/`QueenLia` to unlock Sugar Star for that game session.
...
- Responsive HTML5 canvas that scales to fit phone, tablet, and desktop viewports without page scrolling, pinch zoom, or double-tap zoom interrupting play.
...
- Seven selectable bird characters with the current pick saved on this device when available, including immediately playable birds, score-unlocked birds like Emerald Duck at 30 points, and secret code characters like Six Seven, Logan Clock, and the white/silver Sugar Star that require their secret codes again each time the game page starts.
